            ...
            if ($order->getState() == Order::STATE_PROCESSING && !$order->hasInvoices()) {
                ...
                if (!empty($data['last_trans_id'])) {
                    ...
                    if ($bHasAmountAuthorized && $amountPaid === null && $amountRefunded === null) {
                        payHelper::log('AUTO-CAPTURING (rest)' . $data['last_trans_id'], payHelper::LOG_TYPE_DEBUG, [], $order->getStore());
                        try {
                            \Paynl\Config::setApiToken($this->config->getApiToken());
                            $result = \Paynl\Transaction::capture($data['last_trans_id']);
                            $strResult = 'Success';
                        } catch (\Exception $e) {
                            payHelper::log('Order PAY error (rest): ' . $e->getMessage() . ' EntityId: ' . $order->getEntityId(), payHelper::LOG_TYPE_DEBUG, [], $order->getStore());
